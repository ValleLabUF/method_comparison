...
table(k.optim)/20
# AIC suggested 5 sims w/ 4 states
# BIC suggested 6 sims w/ 4 states
...
# accuracy ranges from 70.4% to 88.3%
...
hmm.states %>% 
  ...
  mutate(acc = n/track_length) %>% 
  summarise(min=min(acc), max=max(acc), mean=mean(acc))
# accuracy ranges from 85.5% to 93.6%
...
# write.csv(hmm.states, "HMM2 results.csv", row.names = F)
# write.csv(time, "HMM2_elapsed_time.csv", row.names = F)  #units = min